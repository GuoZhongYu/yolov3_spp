...
# p代表的是预测的值，是长度为3的列表，每一个值代表的就是在不同尺度的特征图上预测的结果
'''
targets代表的是标注的信息，形状为（x，6）的tensor，x指在这个batch中包含多少个目标，6是指每个目标包含的参数个数
6个参数分别为：targets(image_idx,class,x,y,w,h)
    image_idx：代表的是当前这个目标是这个batch中那一张图片里面的目标
    class：代表的是目标的类别
    x，y，w，h：代表的是该目标的坐标信息，但是该目标的坐标信息是一个相对的目标信息
'''
def build_targets(p, targets, model):
    # Build targets for compute_loss(), input targets(image_idx,class,x,y,w,h)
    # 通过targets.shape[0]来获取当前这个batch中的目标个数，对应于上面（x，6）中的x
    nt = targets.shape[0]
    # 对参数进行初始化
    tcls, tbox, indices, anch = [], [], [], []
    gain = torch.ones(6, device=targets.device)  # normalized to gridspace gain

    # 判断模型是不是多gpu训练，model的类型是不是属于后面的某一个之一，如果是的话模型就是多gpu训练
    multi_gpu = type(model) in (nn.parallel.DataParallel, nn.parallel.DistributedDataParallel)
    # model.yolo_layers获取的是层名为yolo的索引
    for i, j in enumerate(model.yolo_layers):  # j: [89, 101, 113]
        # 获取该yolo predictor对应的anchors模板
        # 注意anchor_vec是anchors缩放到对应特征层上的尺度，这里是缩放到对应尺度之后的anchor
        anchors = model.module.module_list[j].anchor_vec if multi_gpu else model.module_list[j].anchor_vec
        # p[i].shape: [batch_size, 3, grid_h, grid_w, num_params]
        gain[2:] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain
        na = anchors.shape[0]  # number of anchors
        # [3] -> [3, 1] -> [3, nt]
        # repeat，将矩阵按照给定的axis将每个元素重复repeats次数
